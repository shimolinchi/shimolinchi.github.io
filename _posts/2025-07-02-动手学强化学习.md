...
这里面引入了熵$H(X) = \mathbb{E}_{x\sim p}[-\log p(x)]$。这我熟啊，热力学里面的熵嘛。
...
$$V(s_t) = \mathbb{E}_{s_t\sim \pi}[Q(s_t,a_t)-\alpha\log \pi(a_t|s_t)]=\alpha \log \sum_{a} \exp\Big(\frac{Q(s,a)}{\alpha}\Big)$$
...
这个温度参数在存储和优化的时候是以对数的形式进行的，在使用的时候在用指数转换回来，这样用对数进行优化的时候能保证$\alpha$始终是正的，而且在优化的过程也更加稳定

算法流程如下：

![](https://raw.githubusercontent.com/shimolinchi/shimolinchi.github.io/master/img/2025-01-26-机器学习/45.png)

# 模仿学习

就是对于奖励函数不好设置的场景，通过一个专家来对智能体的行为进行学习

## 行为克隆 BC

就是相当于专家生成的数据通过直接监督的学习方法。
优化目标为：
$$\theta^* = \arg \min_\theta\mathbb{E}_{(s,a)}[L(\pi_\theta(s),a)]$$,

其中，B是专家数据集，L是对应监督学习框架下的损失函数。

## 生成式对抗模仿学习 GAIL

类似于 GAN ，有一个生成器（策略网络，相当于Actor），有一个判别器。

引出了占用度量：
$$\rho_\pi(s,a) = (1 - \gamma) \sum_{t=0}^{\infty} \gamma^t \Pr(s_t = s, a_t = a \;|\; \pi)$$
就是该生成器对于状态动作对的概率。

判别器的损失函数为：
$$\mathcal{L}(\phi) = - \mathbb{E}_{\rho{\pi}}[\log D_{\phi}(s,a)] - \mathbb{E}_{\rho{E}}[\log(1 - D_{\phi}(s,a))]$$

就是对于输入的一个状态动作对，如果是智能体的数据，输出接近1，如果是专家数据，输出尽量接近0

这里涉及到了状态动作对的编码，如果离散动作，则编码的时候需要使用独热编码，但是连续动作不需要使用独热编码